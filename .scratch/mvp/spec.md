...
41. As a user, I want Zed's graphical Settings window on `Cmd+,` for discoverable configuration, plus an explicit `Open Settings File` command for the sole JSONC file at `~/.config/<app-name>/settings.json` (native Zed settings at the root and app-only policy under `zednotes`), so that the GUI and plain-text editor always modify the same settings.
42. As a user, I want Zed's full Keymap Editor on `Cmd+K Cmd+S`, backed by the sole `~/.config/<app-name>/keymap.json` using GPUI/Zed key-context conventions, so that I can search every registered action and create, edit, remove, or inspect conflicting bindings without a new keybinding system.
...
- **Settings** use Zed's native JSONC schema at the root of the sole `~/.config/<app-name>/settings.json`, with app-only policy under `zednotes`; `Cmd+,` and `Open Settings File` edit that same file. The sole `keymap.json` is shared by Zed's full Keymap Editor and plain-text editing, using GPUI key contexts (Workspace, Explorer, Editor, Search, CommandPalette, QuickOpen, MarkdownPreview, Dialog). Which-key defaults to enabled at 500 ms.
